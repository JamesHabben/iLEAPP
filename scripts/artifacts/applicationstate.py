...
def get_applicationstate(files_found, report_folder, seeker, wrap_text, timezone_offset):
    ...
    cursor.execute('''
    select 
    ait.application_identifier as ai, 
    kvs.value as compat_info,
    (SELECT 
        kvs.value from kvs 
        left join application_identifier_tab on application_identifier_tab.id = kvs.application_identifier
