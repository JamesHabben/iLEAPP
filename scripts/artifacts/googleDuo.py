...
def get_googleDuo(files_found, report_folder, seeker, wrap_text, timezone_offset):
    ...
    for file_found in files_found:
        ...
        if usageentries > 0:
            ...
            report.add_script()
            data_headers = ('Registration Date','Name','ID','Number Label','Sync Date')
            data_list = []
            for row in all_rows:
                data_list.append((
                    (row[0], 'datetime'),
        ...
        if usageentries > 0:
            ...
            report.add_script()
            data_headers = ('Timestamp','Local User ID','Remote User ID','Contact Name',
        ...
        if usageentries > 0:
            for row in all_rows:
                ...
                data_list.append((
                    (clip_creation, 'datetime'),
                    (clip_message, 'datetime'),
                    (clip_viewed, 'datetime'),
                    (local_id, 'phonenumber'),
            ...
            report.add_script()
            data_headers = (
                'Creation Date', 'Message Date', 'Viewed Date', 'Local User ID',
                'Clip Direction','Text Representation', 'Message ID','Content Size',
                'Transferred Size', 'Clip')
