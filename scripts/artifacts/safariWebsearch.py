...
def get_safariWebsearch(files_found, report_folder, seeker, wrap_text, timezone_offset):
	...
	if usageentries > 0:
		for row in all_rows:
			...
			data_list.append((
				(row[0], 'datetime'),
				search, row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]
			))
		...
		report.add_script()
		data_headers = ('Visit Time','Search Term','URL','Visit Count','Title','iCloud Sync',
						'Load Successful','Visit ID','Redirect Source','Redirect Destination' )
