...
def generate_report(reportfolderbase, time_in_secs, time_HMS, extraction_type, image_input_path, casedata):
    ...
    elements_source = os.path.abspath(os.path.join(__location__, 'report_elements'))
    ...
    # Copy all files from 'elements_source' to the 'elements_folder'
    for filename in os.listdir(elements_source):
        src = os.path.join(elements_source, filename)
        dst = os.path.join(elements_folder, filename)

        if os.path.isdir(src):
            try:
                shutil.copytree(src, dst, copy_function=copy_no_perm)
            except shutil.Error:
